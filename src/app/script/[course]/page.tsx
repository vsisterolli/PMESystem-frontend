...
export default function Script  ({ params }) {
  ...
  const [courseObject, setCourseObject] = useState({document: "", name: "", acronym: ""})

  const router = useRouter();
  useEffect(() => {
    client.get("/departaments/course/" + params.course, {headers: {Authorization: userData?.access_token}})
  ...
  return (
    <>
      ...
      <div className={"flex flex-col items-center justify-center bg-[#261046] " + styles.document}>
        <h1 className={styles.docTitle + " " + alata.className}>{courseObject.acronym} - {courseObject.name}</h1>
