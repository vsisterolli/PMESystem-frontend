...
export default function ChangeCape({ capeSelected, visibilityState }) {
    ...
    async function changeCape(option) {
        ...
        await client.patch(
            "/cosmetic/userCape/",
            { userNick: userData?.userData?.nick, capeName: option },
    ...
    return (
        ...
        >
            ...
            >
                <h3>MINHAS CAPAS</h3>
                <div
                    onClick={() => debounce("base.png")}
                    className={styles.capeOption}
                >
                    <input
                        type="checkbox"
                        checked={"base.png" === chosen}
                        className={styles.checkboxRound}
                    />
                    <div
                        className={
                            styles.changeCapeOption + " bg-[url('/base.png')]"
                        }
                    ></div>
                </div>
                <h3>Capa padrão</h3>
                {capes.map((option) => (
                    <>
                        <div
                            onClick={() => debounce(option.fileName)}
                            className={styles.capeOption}
                        >
                            <input
                                type="checkbox"
                                checked={option.fileName === chosen}
                                className={styles.checkboxRound}
                            />
                            <div
                                style={{
                                    backgroundImage: `url(${baseS3 + option.fileName})`
                                }}
                                className={styles.changeCapeOption}
                            ></div>
                        </div>
                        <h3>{option.name}</h3>
                    </>
